...
class Subscription < ApplicationRecord
  ...
  has_one :organization, through: :owner
  ...
  validates :billing_time, presence: true
  ...
  # NOTE: SQL query to get subscription_at into owner timezone
  def self.subscription_at_in_timezone_sql
    <<-SQL
      subscriptions.subscription_at::timestamptz AT TIME ZONE
      COALESCE(owners.timezone, organizations.timezone, 'UTC')
    SQL
  end
  ...
  def initial_started_at
    owner.subscriptions.where.not(started_at: nil).order(started_at: :asc).first&.started_at || subscription_at
  end
